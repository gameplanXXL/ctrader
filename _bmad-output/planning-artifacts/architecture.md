...
1. **Data Ingestion, Order Placement & Persistence (FR1–7, FR14–18b, FR49–52, FR53–58):** Zwei Broker-Datenquellen (IB Flex XML + Live-Stream, cTrader Protobuf), **IB Swing-Order-Submission via `ib_async` Bracket Orders mit festem Stop-Loss für Aktien und Single-Leg-Optionen (Long/Short Call/Put)** — Epic 12 am Ende der Pipeline, eine Taxonomie-Quelle (`taxonomy.yaml`), ein MCP-Contract-Snapshot, und Scheduled Jobs für Nightly-Reconciliation, Regime-Snapshots und Backups. Zentral: JSONB `trigger_spec` als strukturiertes Provenance-Schema. **Architektur-Implikation:** Broker-Abstraktionsschicht, idempotente Ingestion-Pipeline (mit `orderRef`-basierter Order-Idempotenz für Quick-Orders), PostgreSQL JSONB als First-Class-Concern, dedizierter `order_service` mit Stock- und Option-Contract-Pfad für den Order-Lifecycle.
...
9. **IB Swing-Order via `ib_async` Bracket Order mit festem Stop-Loss** (FR53–58, Epic 12 am Ende der Pipeline) — Aktien + Single-Leg-Optionen (Long/Short Call/Put, Fokus Short-Verkauf), dedizierter `order_service.py` mit Stock- und Option-Contract-Qualifizierung (`ib_async.Option` + `reqContractDetails`), `whatIfOrder`-Margin-Check für Short-Options, Endpoint unter `routers/trades.py` (`POST /trades/quick-order` mit `asset_class` field), erweiterte `trades`-Tabelle mit Order-Lifecycle-Spalten und Option-Feldern (expiry, strike, right, multiplier, asset_class), Auto-Tag `near-assignment` bei Expiry < 3 Tagen oder ITM, Kill-Switch-Exemption für manuelle Quick-Orders
